...
namespace SummerJam1
{
    public class TextPopupManager : MonoBehaviourSingleton<TextPopupManager>
    {
        ...
        private UltimateTextDamageManager UiManager => _manager != null
            ? _manager
            : _manager = FindObjectsOfType<UltimateTextDamageManager>(true).FirstOrDefault(utd => !utd.convertToCamera);

        private UltimateTextDamageManager WorldManager => _worldManager != null
            ? _worldManager
            : _worldManager = FindObjectsOfType<UltimateTextDamageManager>(true).FirstOrDefault(utd => utd.convertToCamera);
