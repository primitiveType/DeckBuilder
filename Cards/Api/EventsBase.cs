...
namespace Api
{
    public abstract class EventsBase : object
    {
        ...
        {
            EventHandle<EntityCreatedEventArgs> handler = new EventHandle<EntityCreatedEventArgs>(action, () => EntityCreated -= action);
        ...
        public override IDisposable GetEventHandle(MethodInfo attached, IComponent instance, EventsBase events)
        {
                ...
                parameters[1].ParameterType != typeof(EntityCreatedEventArgs))
            {
                throw new NotSupportedException(
                    "Wrong parameters for attribute usage! must match signature (object sender, EntityCreatedEventArgs) args)");
            }
