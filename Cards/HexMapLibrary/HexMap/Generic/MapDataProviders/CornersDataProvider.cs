...
namespace Wunderwunsch.HexMapLibrary.Generic
{
    public class CornersDataProvider<C> where C : new()
    {
        ...
        private readonly CornersPositionsProvider cornersPositionsProvider;
        ...
        /// <summary>
        ///     returns the shortest path of corners from the origin coordinate to the target corner coordinate - optionally
        ///     including the origin
        /// </summary>
        public List<Corner<C>> PathAlongGrid(Vector3Int originCorner, Vector3Int targetCorner, bool includeOrigin,
            float horizontalNudgeFromOriginCenter = NudgePositive)
        {
            List<Vector3Int> positions =
                cornersPositionsProvider.PathAlongGrid(originCorner, targetCorner, includeOrigin, horizontalNudgeFromOriginCenter);
            return GetCornersFromCoordinates(positions);
        }
