...
namespace Wunderwunsch.HexMapLibrary
{
    public class CornersPositionsProvider
    {
        ...
        public List<Vector3Int> PathAlongGrid(Vector3Int originCorner, Vector3Int targetCorner, bool includeOrigin,
            float horizontalNudgeFromOriginCenter = NudgePositive)
        {
            ...
            List<Vector3Int> cornerCoordinates =
                HexGrid.GetCorners.PathAlongGrid(originCorner, targetCorner, includeOrigin, horizontalNudgeFromOriginCenter);
            if (coordinateWrapper != null)
            {
                cornerCoordinates = coordinateWrapper.WrapCornerCoordinates(cornerCoordinates);
            }
        ...
        public List<Vector3Int> TileBorders(IEnumerable<Vector3Int> tiles)
        {
            ...
            if (coordinateWrapper != null)
            {
                corners = coordinateWrapper.WrapCornerCoordinates(corners);
            }
        ...
        /// <summary>
        ///     Wraps the input corner positions (if the map is periodic) and removes all those which are still out of the map
        ///     bounds after that
        /// </summary>
        protected List<Vector3Int> GetValidCornerCoordinates(List<Vector3Int> rawPositions)
        {
            ...
            if (coordinateWrapper != null)
            {
                positions = coordinateWrapper.WrapCornerCoordinates(positions);
            }
