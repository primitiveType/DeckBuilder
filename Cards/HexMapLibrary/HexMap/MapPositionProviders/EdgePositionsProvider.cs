...
namespace Wunderwunsch.HexMapLibrary
{
    public class EdgePositionsProvider
    {
        ...
        public List<Vector3Int> OfTile(Vector3Int tile)
        {
            ...
            {
                edges = coordinateWrapper.WrapEdgeCoordinates(edges);
            }
        ...
        /// <summary>
        ///     returns a list of all edges which form the border(s) of the input Tile Coordinates. They are returned in an
        ///     arbitrary order and might belong to different border paths.
        /// </summary>
        ...
        public List<Vector3Int> TileBorders(IEnumerable<Vector3Int> tiles)
        {
            ...
            foreach (Vector3Int tile in tiles)
            {
                ...
                foreach (Vector3Int c in edgeCoordsOfCell)
                {
                    if (edges.Contains(c))
                    {
                        edges.Remove(c);
        ...
        public List<Vector3Int> PathBetweenCorners(Vector3Int originCorner, Vector3Int targetCorner,
            float horizontalNudgeFromOriginCenter = NudgePositive)
        {
            ...
            List<Vector3Int> edges = HexGrid.GetEdges.PathBetweenCorners(originCorner, targetCorner, horizontalNudgeFromOriginCenter);
            return GetValidEdgeCoordinates(edges);
            //throw new System.NotImplementedException();
        }

        /// <summary>
        ///     returns an list of each contiguous border path of the input Tile Coordinates. Each individual path is ordered in
        ///     clockwise direction with an arbitrary starting point. The different Paths are returned in an arbitrary order
        /// </summary>
        ...
        public List<List<Vector3Int>> BorderPaths(IEnumerable<Vector3Int> tiles, out List<List<EdgeDirection>> pathDirections)
        {
            ...
            HashSet<Vector3Int>
                unusedEdges = new HashSet<Vector3Int>(
                    edgesUnordered); //we remove every edge which we used from this collection to find out which are still left after we finished a path
