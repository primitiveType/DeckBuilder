...
namespace Wunderwunsch.HexMapLibrary
{
    public class TilePositionsProvider
    {
        ...
        public List<List<Vector3Int>> ContiguousAreasOfInputTiles(ICollection<Vector3Int> inputTiles)
        {
            ...
            while (unusedTiles.Count > 0)
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    List<Vector3Int>
                        neighbours = AdjacentToTile(current); //only that line is different from static version, should refactor at some point
        ...
        protected List<Vector3Int> GetValidTileCoordinates(List<Vector3Int> rawPositions)
        {
            ...
            if (coordinateWrapper != null)
            {
                positions = coordinateWrapper.WrapTileCoordinates(positions);
            }
