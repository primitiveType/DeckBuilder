...
namespace Wunderwunsch.HexMapLibrary
{
    public class EdgePositionProvider
    {
        ...
        public EdgePositionProvider(CoordinateWrapper coordinateWrapper, Dictionary<Vector3Int, int> EdgeIndexByPosition)
        {
            ...
            edgeIndexByPosition = EdgeIndexByPosition;
        ...
        /// <summary>
        ///     returns the periodic edge coordinate of the input cartesian coordinate, the out parameter specifies if that
        ...
        public Vector3Int FromCartesianCoordinate(Vector3 cartesianCoordinate, out bool edgeIsOnMap)
        {
            ...
            Vector3Int coord = HexConverter.CartesianCoordToClosestEdgeCoord(cartesianCoordinate);
            edgeIsOnMap = false;
            if (edgeIndexByPosition.ContainsKey(coord))
            {
                edgeIsOnMap = true;
            }
        ...
        {
            if (GetCornerDistance.Grid(cornerA, cornerA) != 1)
            {
                throw new ArgumentException("Corners don't have a distance of 1, therefore and not neighbours and share no Edge");
            }

            Vector3Int cornerCoordinate = new Vector3Int((cornerA.x + cornerB.x) / 3, (cornerA.y + cornerB.y) / 3, (cornerA.z + cornerB.z) / 3);
            if (coordinateWrapper != null)
            {
                cornerCoordinate = coordinateWrapper.WrapCornerCoordinate(cornerCoordinate);
            }
        ...
        public Vector3Int BetweenNeighbouringTiles(Vector3Int tileA, Vector3Int tileB)
        {
            ...
            {
                throw new ArgumentException("Tiles don't have a distance of 1, therefore and not neighbours and share no Edge");
            }
