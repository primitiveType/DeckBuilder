        ...
        public static class GetEdges
        {
            ...
            public static List<Vector3Int> WithinDistanceOfEdge(Vector3Int centerEdge, int maxDistance, bool includeCenter)
            {
                ...
                for (int i = 0; i <= maxDistance; i++)
                {
                    ...
                    foreach (Vector3Int openEdge in openEdges)
                    {
                        edgesInRange.Add(openEdge);
                        List<Vector3Int> adjacent = AdjacentToEdge(openEdge);
            ...
            public static List<Vector3Int> WithinDistanceOfCorner(Vector3Int centerCorner, int maxDistance)
            {
                ...
                List<Vector3Int> adjacentToCorner = AdjacentToCorner(centerCorner);
                adjacentToCorner.ForEach(x => openEdges.Add(x));

                for (int i = 1; i <= maxDistance; i++)
                {
                    HashSet<Vector3Int> edgesWithDistanceI = new HashSet<Vector3Int>();
                    foreach (Vector3Int edge in openEdges)
                    {
                        edgesInRange.Add(edge);
                        List<Vector3Int> adjacent = AdjacentToEdge(edge);
                        foreach (Vector3Int adj in adjacent)
                        {
                            if (!edgesInRange.Contains(adj))
                            {
            ...
            public static List<Vector3Int> AtExactDistance(Vector3Int centeredge, int distance)
            {
                ...
                foreach (Vector3Int edge in allWithinDistance)
                {
                    int dist = GetDistance.BetweenEdges(edge, centeredge);
                    if (dist == distance)
                    {
                        atExactDistance.Add(edge);
                    }
                }
            ...
            public static List<Vector3Int> PathBetweenCorners(Vector3Int originCorner, Vector3Int targetCorner,
                float horizontalNudgeFromOriginCenter = NudgePositive)
            {
                ...
                {
                    edges.Add(GetEdge.BetweenCorners(corners[i], corners[i + 1]));
                }
