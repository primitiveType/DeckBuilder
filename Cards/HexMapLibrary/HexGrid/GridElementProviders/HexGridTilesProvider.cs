        ...
        public static class GetTiles
        {
            ...
            public static List<Vector3Int> Disc(Vector3Int center, int radius, bool includeCenter)
            {
                ...
                for (int x = minX; x <= maxX; x++)
                {
                    ...
                    {
                        int z = -x - y;
                        if (Mathf.Abs(z - center.z) > radius)
                        {
                            continue;
                        }
            ...
            public static List<Vector3Int> Ring(Vector3Int center, int radius, int ringThicknessInwards)
            {
                ...
                {
                    throw new ArgumentException("ring thickness must be larger than 0");
                }

                List<Vector3Int> ring = new List<Vector3Int>();
                List<Vector3Int> allInManhattanrange = Disc(center, radius, true);
                foreach (Vector3Int v in allInManhattanrange)
                {
            ...
            /// </summary>
            public static List<Vector3Int> Ring(Vector3Int center, int radius, TileDirection startDirection = TileDirection.TopRight,
                bool clockwise = true)
            {
                if (radius < 1)
                {
                ...
                Func<Vector3Int, Vector3Int, Vector3Int> rotationMethod;
                if (clockwise)
                {
                ...
                for (int i = 0; i < 6; i++)
                {
                    List<Vector3Int> tilesOfRingSegment = Line(ringCornerTiles[i], ringCornerTiles[(i + 1) % 6], false);
            ...
            public static List<Vector3Int> Cone(Vector3Int origin, Vector3Int targetDirection, float coneWidthHalfAngle, int coneLength)
            {
                ...
                IEnumerable<Vector3Int> ring = Ring(origin, coneLength, 1);
                ...
                foreach (Vector3Int target in ring)
                {
                    ...
                    List<Vector3Int>
                        linePointsL = Line(origin, target, false,
                            -0.00001f); //for more consistent results we use 2 lines, one slightly left of center
                    List<Vector3Int> linePointsR = Line(origin, target, false, +0.00001f); //and this her slightly right of center
            ...
            public static List<Vector3Int> Line(Vector3Int origin, Vector3Int target, bool includeOrigin,
            ...
            public static List<List<Vector3Int>> ContiguousAreasOfInputTiles(ICollection<Vector3Int> inputTiles)
            {
                ...
                while (unusedTiles.Count > 0)
                {
                    ...
                    while (queue.Count > 0)
                    {
                        ...
                        List<Vector3Int> neighbours = AdjacentToTile(current);
                        neighbours.RemoveAll(x => !unusedTiles.Contains(x));
                        foreach (Vector3Int tile in neighbours)
                        {
